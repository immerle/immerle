 ...
 *
 * Subsonic token authentication requires `token = md5(password + salt)`.
 * We need this to run identically on web, iOS and Android without pulling a
 * native crypto module, so a small pure-JS implementation lives here. It is
 * only ever used to derive the auth token at login time — the raw password is
 * never persisted (see `@/auth`).
 *
