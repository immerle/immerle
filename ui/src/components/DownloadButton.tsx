...
import { useOfflineCatalog } from '../offline/catalog';
import { IconButton } from './ui';
import { useColors } from '../theme/colors';
import { useT } from '../i18n/store';

/** Passed by the album/playlist screens so downloading "the whole thing"
 * also snapshots its metadata — a lone track download only saves the song. */
type Snapshot =
  | { type: 'album'; id: string; name: string; artist?: string; artistId?: string; year?: number; coverArt?: string }
 ...
 */
export function DownloadButton({ songs, size = 24, snapshot }: { songs: Song[]; size?: number; snapshot?: Snapshot }) {
  const t = useT();
  ...
  const saveAlbum = useOfflineCatalog((s) => s.saveAlbum);
  const savePlaylist = useOfflineCatalog((s) => s.savePlaylist);

  if (!canOffline || songs.length === 0) return null;
  const allDownloaded = songs.every((s) => entries[s.id]);

  const download = async () => {
    await downloadMany(songs);
    if (!snapshot) return;
    if (snapshot.type === 'album') saveAlbum({ ...snapshot, songs });
    else savePlaylist({ ...snapshot, songs });
  };
  ...
  return (
    <IconButton
      ...
      onPress={() => {
        if (!allDownloaded) void download();
      }}
