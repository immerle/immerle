...
// Song/Playlist/Radio is set, per Type.
type searchHitView struct {
	Type     string        `json:"type"` // artist|album|song|playlist|radio
	Artist   *artistView   `json:"artist,omitempty"`
	Album    *albumView    `json:"album,omitempty"`
	Song     *songView     `json:"song,omitempty"`
	Playlist *playlistView `json:"playlist,omitempty"`
	Radio    *stationView  `json:"radio,omitempty"`
}

// searchView is the result of a catalog search: every match — artists,
// albums, songs, public playlists and radio stations alike — in one list,
// ranked by relevance to the query rather than grouped by type.
type searchView struct {
	Results []searchHitView `json:"results"`
}

// maxSearchRadio caps the radio stations returned by a global search.
const maxSearchRadio = 10
...
func searchCounts(r *http.Request) (artistCount, albumCount, songCount, playlistCount int) {
	...
	case "radio":
		artistCount, albumCount, songCount, playlistCount = 0, 0, 0, 0
	}
	return
}
...
// @Param        type         query  string  false  "Scope to one result type: artist, album, song, playlist or radio (default: all)"
...
func (h *Handler) handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" && (searchType == "" || searchType == "radio") && h.radioEnabled() && h.Radio != nil {
		if stations, err := h.Radio.ListForUser(r.Context(), userFrom(r.Context()).ID); err == nil {
			sort.SliceStable(stations, func(i, j int) bool {
				return core.Relevance(q, stations[i].Name) < core.Relevance(q, stations[j].Name)
			})
			matched := 0
			for _, s := range stations {
				if matched >= maxSearchRadio || core.Relevance(q, s.Name) >= 3 {
					break
				}
				v := toStationView(s)
				hits = append(hits, rankedHit{s.Name, searchHitView{Type: "radio", Radio: &v}})
				matched++
			}
		}
	}
