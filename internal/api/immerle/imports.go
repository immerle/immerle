...
func (h *Handler) handleImportStart(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.Logger.Warn("import start failed", "source", req.Source, "error", err)
		writeError(w, http.StatusBadRequest, "bad_request", "could not start import")
		return
	}
...
func (h *Handler) handleImportItemResolve(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		h.Logger.Warn("import item resolve failed", "item", itemID, "error", err)
		writeError(w, http.StatusBadRequest, "bad_request", "could not resolve import item")
		return
	}
